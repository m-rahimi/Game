...
class CardGame(arcade.Window):
    ...
    def computer_move(self):
        ...
        if not visible_cards:
            return
        player1 = [x[0] if x else None for x in self.player1_cards]
        player2 = [x[0] if x else None for x in self.player2_cards]
        floor = self.floor_cards[:] 
        best, id = min_max(player1, player2, floor, player=1, depth=0, max_depth=4, score1=0, score2=0)
        print(f"Computer move: {best}, id: {id}")
        card = self.player1_cards[id][0] 
#        i, card = random.choice(visible_cards)
